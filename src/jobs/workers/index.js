...
// scheduler.queue.js, queues/news.queue.js, queues/delayAttribution.queue.js)
// — must happen once per process start, before the corresponding Workers
// below are created, so each has a tick to fire on.
require("../queues/scheduler.queue");
require("../queues/news.queue");
require("../queues/delayAttribution.queue");
...
const delayAttributionWorker = require("./delayAttribution.worker");
...
logger.info(
  "BullMQ workers started: scheduler, ingestion, normalize-and-correlate, news, delay-attribution"
);
